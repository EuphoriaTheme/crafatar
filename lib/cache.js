...
function disable_redis_client(reason, client) {
  var current = client || redis;
  redis = null;
  enable_memory_cache(reason);
  try {
...
function connect_redis() {
  ...
  redis.on("error", function(err) {
    ...
    if (/Unknown RESP type/i.test(message) || /WRONGPASS|NOAUTH/i.test(message)) {
      ...
      disable_redis_client("redis auth/protocol error", redis);
      return;
    }
    if (
      !(redis && redis.isOpen) &&
      (/AggregateError/i.test(message) || /ECONNREFUSED|ENOTFOUND|EAI_AGAIN|ETIMEDOUT|ECONNRESET/i.test(message))
    ) {
      logging.warn("Redis unavailable; disabling Redis cache.");
      disable_redis_client("redis unavailable", redis);
      return;
    }
    var now = Date.now();
  ...
  redis.connect().catch(function(err) {
    logging.error(err);
    disable_redis_client("redis connect failed", redis);
  });
}
