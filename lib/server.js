...
function getAssetCacheControl(filename) {
  var ext = path.extname(filename).toLowerCase();
  if (ext === ".js" || ext === ".css" || ext === ".map") {
    return "no-cache, max-age=0";
  }
  return "max-age=" + config.caching.browser;
}
      ...
      if (!fs_err) {
        fs.readFile(filename, function(err, data) {
          callback({
            ...
            cache_control: getAssetCacheControl(filename),
            err: err,
          });
        });
      } else {
...
function requestHandler(req, res) {
  ...
  try {
    req.url = new URL(decodedUrl, 'http://' + host);
  } catch (e) {
    response(req, res, {
      status: -2,
      body: "Bad Request",
  ...
  req.url.pathname = path.posix.normalize(req.url.pathname || "/");
  if (req.url.pathname.charAt(0) !== "/") {
  ...
  if (req.method === "GET" || req.method === "HEAD") {
    try {
      ...
      case "capes":
        ...
        break;
      case "status":
        routes.status(req, function(result) {
          response(req, res, result);
        });
        break;
      default:
